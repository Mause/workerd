    ...
    ($marker:ident, $elem:ty, $get_fn:ident) => {
        impl<'a> Local<'a, $marker> {
            ...
            pub fn as_mut_slice(&mut self) -> &mut [$elem] {
                ...
                // SAFETY: handle is valid; non-empty guarantees non-null data pointer.
                // &mut self prevents aliasing through *this* handle. Note: two distinct
                // Local handles backed by the same ArrayBuffer could alias — callers must
                // ensure no other slice into this buffer region is live.
                unsafe {
                    let ptr = self.data().byte_add(self.byte_offset());
